# Plan Risk Review: Phase 05 — KB Generator (05-01-PLAN.md)
...
- `src/github_pr_kb/generator.py` — new KBGenerator class, slugify function, GenerateResult model
- `src/github_pr_kb/config.py` — adding `kb_output_dir` field to Settings
- `tests/test_generator.py` — full TDD test suite
...
- **Implicit.** The plan's frontmatter spec (Task 2, step h) calls for `comment_url (comment.url)`. CommentRecord has a `url` field (models.py:16) — this resolves to the comment's URL. **Justified.** No risk here.
...
- **Justified?** Mostly — classifier.py reads `pr-N.json` to produce `classified-pr-N.json`, so if the classified file exists, the PR file was there at classification time. But it could have been deleted since. The plan does handle this: "On error, log warning and continue."
...
- **Justified?** The plan actually specifies parsing `ClassifiedFile` first, then using `pr.number` to find the PR file. Wait — re-reading Task 2 step j more carefully: it says "for each classified file path" → parse → then load `pr-{N}.json`. The `N` would come from `ClassifiedFile.pr.number`. **Adequate**, though the plan could be more explicit about this link.
- **Classification:** Peripheral.
...
- **Justified?** Yes — `conftest.py` sets env vars at module level, but the lazy import pattern is a belt-and-suspenders approach. The classifier does the same thing (classifier.py:82-83).
- **Classification:** Peripheral.

### A5: YAML frontmatter doesn't need a parser — f-string construction is safe
- **Explicit** (Research doc). Claims that manually-constructed frontmatter with `_yaml_str()` quoting is sufficient.
- **Justified?** For this use case, yes — the fields are known and controlled. But `_yaml_str` only escapes double quotes. What about values containing newlines? A PR title like `"Fix bug\nin middleware"` would break the frontmatter.
...
- **Testable?** Yes — add a test with a newline in pr_title.
...
- **Justified?** Yes — GitHub comment IDs are globally unique integers across all repositories. This is a well-known GitHub API property.
- **Classification:** Peripheral.
...
- **Justified?** Yes — the plan is scoped to Plan 01 only. But this means the generator will work without producing INDEX.md. This is fine if there's a Plan 02 to follow.
- **Classification:** Peripheral.
...
**A new engineer joining the team:** The `comment_url` field in frontmatter is confusing because the model field is just called `url`. The frontmatter has `pr_url` (from `pr.url`) and `comment_url` (from `comment.url`) — but looking at the code, a newcomer might not immediately see which `url` goes where. The plan is clear enough, but the generated articles would benefit from this being documented somewhere.
...
**A user who doesn't read changelogs:** The plan produces articles with `needs_review: true` in frontmatter but provides no mechanism for a user to *act* on that flag — no CLI command to list needs-review articles, no workflow guidance. This is fine for Plan 01 (out of scope), but worth noting as a gap in the overall system.
...
| R5 | Schedule | Plan 01 covers KB-01, KB-02, KB-04 but not KB-03 (INDEX.md) — if Plan 02 is delayed, the KB has no index | Plan 02 execution is deferred or blocked | Low | Low | Low | No INDEX.md file after running generate | This is by design — the plan is scoped deliberately | Implement a minimal index in Plan 01 as stretch goal | A8 |

**Known Knowns:** R3, R4 — well-understood, low-impact risks with established mitigations.
**Known Unknowns:** R1 — we don't know the distribution of special characters in real PR titles. A test would resolve this (it's a *secret*, not a *mystery*).
**Unknown Knowns:** R2 — the concept of "user edits KB articles" isn't addressed anywhere in the plan documents.
...
1. **R1 — YAML frontmatter quoting (Medium):** The `_yaml_str` helper only escapes double quotes. Add escaping for `\n` → `\\n` and `\` → `\\` in the implementation. Alternatively, strip newlines from PR titles since they're rare and meaningless in a title context.

2. **R2 — Manifest deletion re-generation (Medium):** Not urgent for Plan 01, but worth a sentence in future documentation. The manifest is the single source of dedup truth — its loss means full re-gen.

3. **R3 — Windows file locking (Low):** The development environment is Windows. The `_write_atomic` pattern should handle `PermissionError` gracefully per article, not crash the run. The plan's error-handling flow (D-18/D-19) already covers this if the exception propagation is correct.

### Recommended Actions

- **Before implementation:** Add a test case for `_yaml_str` with a PR title containing a newline character. This is a *secret* — one test resolves it.
...
- **Scope discipline.** The plan deliberately excludes KB-03 (INDEX.md) to keep Plan 01 focused. This is the right call — shipping articles + manifest first, index second.
